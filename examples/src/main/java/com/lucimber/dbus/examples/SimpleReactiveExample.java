 ...
 * <p>
 * This example demonstrates basic reactive programming patterns with D-Bus Client Java,
 * focusing on CompletableFuture composition, async operations, and error handling.
 * <p>
 * Key Patterns Demonstrated:
 * - CompletableFuture chaining and composition
 * - Async method call pipelines
...
public class SimpleReactiveExample {
    ...
    private void demonstrateAsyncComposition() throws Exception {
        ...
        try {
            ...
            CompletableFuture<String> result = connection.connect()
                .thenCompose(v -> {
                    System.out.println("   🔗 Connected to D-Bus");
                    return getBusId(connection);
                })
                .thenCompose(busId -> {
                    System.out.println("   🆔 Got bus ID: " + busId.substring(0, Math.min(8, busId.length())) + "...");
    ...
    private void demonstrateErrorRecovery() throws Exception {
        ...
        try {
            ...
            CompletableFuture<String> withFallback = performRiskyOperation(connection)
                .exceptionally(error -> {
                    System.out.println("   ⚠️ Primary operation failed, using fallback");
    ...
    private void demonstrateTimeoutHandling() throws Exception {
        ...
        try {
            ...
            CompletableFuture<String> withTimeout = getBusId(connection)
                .orTimeout(2, TimeUnit.SECONDS)
                .handle((result, error) -> {
                    if (error != null) {
                        System.out.println("   ⏰ Operation timed out, cleaning up...");

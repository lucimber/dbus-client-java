 ...
 *
 * <p>This example shows how to: 1. Connect to the D-Bus session bus 2. Call standard D-Bus methods
 * (introspection, properties, ping) 3. Handle responses and errors
 */
public class SimpleClientExample {
    ...
    private static void testIntrospection(
            Connection connection, String service, String objectPath) {
        try {
    ...
    private static void testPing(Connection connection, String service) {
        try {
            ...
            InboundMessage response =
                    connection.sendRequest(pingCall).toCompletableFuture().get(5, TimeUnit.SECONDS);
    ...
    private static void listServices(Connection connection) {
        try {
            ...
            if (response instanceof InboundMethodReturn methodReturn) {
                ...
                System.err.println(
                        "❌ Failed to list services: " + error.getErrorName().getDelegate());
            }
        } catch (Exception e) {
    ...
    private static void testAnnotationService(Connection connection) {
        try {
            ...
            System.out.println(
                    "ℹ️  Annotation service not available (run SimpleAnnotationExample first)");
        }
    }

    private static void testAnnotationServiceProperties(
            Connection connection, String service, String objectPath) {
        try {
            System.out.println("📊 Getting all properties from annotation service...");

            OutboundMethodCall getAllPropsCall =
                    OutboundMethodCall.Builder.create()
                            .withPath(DBusObjectPath.valueOf(objectPath))
                            ...
                            .withBody(
                                    ...
                                    Arrays.asList(DBusString.valueOf("com.example.Calculator")))
                            .withReplyExpected(true)
                            .build();

            InboundMessage response =
                    connection
                            .sendRequest(getAllPropsCall)
                            .toCompletableFuture()
                            .get(5, TimeUnit.SECONDS);
            ...
            if (response instanceof InboundMethodReturn methodReturn) {
                ...
                if (!payload.isEmpty() && payload.get(0) instanceof DBusDict) {
                    @SuppressWarnings("unchecked")
                    DBusDict<DBusString, DBusVariant> properties =
                            (DBusDict<DBusString, DBusVariant>) payload.get(0);
                ...
                System.err.println(
                        "❌ Failed to get properties: " + error.getErrorName().getDelegate());
            }

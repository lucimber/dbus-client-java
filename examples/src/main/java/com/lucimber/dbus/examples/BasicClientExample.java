 ...
 * <p>
 * This example demonstrates fundamental D-Bus operations including:
...
public class BasicClientExample {
  ...
  private Connection demonstrateConnectionManagement() throws Exception {
    ...
    Duration connectTimeout = Duration.ofSeconds(
            Integer.parseInt(System.getenv().getOrDefault("DBUS_CONNECT_TIMEOUT", "10"))
    );
    Duration methodTimeout = Duration.ofSeconds(
            Integer.parseInt(System.getenv().getOrDefault("DBUS_METHOD_TIMEOUT", "30"))
    );
    ...
    Connection connection = "SESSION".equalsIgnoreCase(busType)
    ...
    connection.connect().toCompletableFuture().get(connectTimeout.toMillis(), TimeUnit.MILLISECONDS);
  ...
  private void validateEnvironmentVariables() {
    ...
    if (systemBusAddress != null) {
      ...
      System.out.println("   ℹ️  DBUS_SYSTEM_BUS_ADDRESS: not set (using default: /var/run/dbus/system_bus_socket)");
    }
    ...
    if (userHome != null) {
      ...
      if (keyringDir.exists()) {
        ...
        System.out.println("   ⚠️  D-Bus keyring directory not found: " + keyringDir.getAbsolutePath() + " (needed for DBUS_COOKIE_SHA1)");
      }
    }
  ...
  private void demonstrateBasicMethodCalls(Connection connection) throws Exception {
    ...
    OutboundMethodCall getIdCall = OutboundMethodCall.Builder
            .create()
    ...
    InboundMessage idResponse = connection.sendRequest(getIdCall)
            .toCompletableFuture()
    ...
    if (idResponse instanceof InboundMethodReturn methodReturn) {
      ...
      if (!payload.isEmpty() && payload.get(0) instanceof DBusString) {
        ...
        System.out.println("   └─ ID Format: " + (daemonId.matches("[0-9a-fA-F]+") ? "Hexadecimal" : "Mixed"));
      } else {
        System.out.println("   └─ ⚠️  Could not parse daemon ID from response");
      }
    } else {
      System.out.println("❌ Unexpected response type: " + idResponse.getClass().getSimpleName());
    }
    ...
    InboundMessage namesResponse = connection.sendRequest(listNamesCall)
            .toCompletableFuture()
            .get(10, TimeUnit.SECONDS);
  ...
  private void demonstrateAsyncOperations(Connection connection) throws Exception {
    ...
    OutboundMethodCall asyncCall = OutboundMethodCall.Builder
            .create()
            .withPath(DBusObjectPath.valueOf(DBUS_PATH))
            .withInterface(DBusString.valueOf(DBUS_INTERFACE))
            .withMember(DBusString.valueOf("GetId"))
    ...
    CompletableFuture<String> result = completionStage
            .thenApply(response -> {
  ...
  private void demonstrateErrorHandling(Connection connection) {
    ...
    try {
      OutboundMethodCall errorCall = OutboundMethodCall.Builder
              .create()
      ...
      InboundMessage response = connection.sendRequest(errorCall)
              .toCompletableFuture()
      ...
      if (response instanceof InboundError error) {
        ...
        System.out.println("⚠️ Expected error response, got: " + response.getClass().getSimpleName());
      }

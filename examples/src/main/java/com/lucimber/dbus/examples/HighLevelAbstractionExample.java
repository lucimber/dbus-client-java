 ...
 * <p>
 * This example demonstrates the new high-level abstractions that simplify
 * D-Bus programming with service proxies and promise-style async handling.
 */
public class HighLevelAbstractionExample {
    ...
    private static void demonstrateServiceProxy(Connection connection) throws Exception {
        ...
        DBusService dbusService = ServiceProxy.create(
            connection,
    ...
    private static void demonstratePromises(Connection connection) throws Exception {
        ...
        DBusPromise.from(connection.sendRequest(invalidCall))
            ...
            .exceptionally(error -> {
                System.out.println("  ✅ Error handled gracefully: " + error.getMessage());
                return null;
            })
            .get(); // Wait for completion

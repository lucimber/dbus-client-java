...
class StandardInterfaceHandlerTest {
  ...
  @Mock
  private InboundMethodCall methodCall;

  @Mock
  private InboundSignal signal;

  @Captor
  private ArgumentCaptor<OutboundMethodReturn> returnCaptor;

  @Captor
  private ArgumentCaptor<OutboundError> errorCaptor;
  ...
  void testIntrospectMethod() {
    ...
    when(methodCall.getObjectPath()).thenReturn(DBusObjectPath.valueOf("/com/example/TestService"));
    when(methodCall.getInterfaceName()).thenReturn(
        java.util.Optional.of(DBusString.valueOf("org.freedesktop.DBus.Introspectable")));
    ...
    verify(context).propagateOutboundMessage(returnCaptor.capture(), any(CompletableFuture.class));
  ...
  void testIntrospectUnknownMethod() {
    ...
    verify(context).propagateOutboundMessage(errorCaptor.capture(), any(CompletableFuture.class));
  ...
  void testPropertiesGet() {
    ...
    when(methodCall.getObjectPath()).thenReturn(DBusObjectPath.valueOf("/com/example/TestService"));
    when(methodCall.getInterfaceName()).thenReturn(
        java.util.Optional.of(DBusString.valueOf("org.freedesktop.DBus.Properties")));
    when(methodCall.getMember()).thenReturn(DBusString.valueOf("Get"));
    when(methodCall.getSerial()).thenReturn(DBusUInt32.valueOf(44));
    when(methodCall.getPayload()).thenReturn(Arrays.asList(
  ...
  void testPropertiesGetAll() {
    ...
    DBusDict<DBusString, DBusVariant> dict = (DBusDict<DBusString, DBusVariant>) reply.getPayload().get(0);
  ...
  void testPropertiesGetAllUnknownInterface() {
    ...
    when(methodCall.getPayload()).thenReturn(Arrays.asList(
        DBusString.valueOf("com.unknown.Interface")
    ));
  ...
  void testPeerPing() {
    ...
    when(methodCall.getObjectPath()).thenReturn(DBusObjectPath.valueOf("/com/example/TestService"));
    when(methodCall.getInterfaceName()).thenReturn(
        java.util.Optional.of(DBusString.valueOf("org.freedesktop.DBus.Peer")));
  ...
  void testUnknownInterface() {
    ...
    when(methodCall.getObjectPath()).thenReturn(DBusObjectPath.valueOf("/com/example/TestService"));
    when(methodCall.getInterfaceName()).thenReturn(
        java.util.Optional.of(DBusString.valueOf("com.unknown.Interface")));
  ...
  void testIntegerPropertyConversion() {
    ...
    StandardInterfaceHandler handlerWithTypes = 
        new StandardInterfaceHandler("/test", service);
  ...
  static class TestServiceWithTypes {
    @DBusProperty
    private Integer count = 42;
    
    @DBusProperty
    private Boolean active = false;
    
    @DBusProperty
    private Object complexObject = new Object() {
      @Override

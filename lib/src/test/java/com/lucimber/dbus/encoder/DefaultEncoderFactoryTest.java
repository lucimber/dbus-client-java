...
class DefaultEncoderFactoryTest {
  ...
  void testCreateEncoderForBasicTypes() throws EncoderException {
    for (Type type : Type.values()) {
      if (factory.canEncode(type)) {
        Encoder<DBusType, ByteBuffer> encoder = factory.createEncoder(type, ByteOrder.LITTLE_ENDIAN);
  ...
  void testCreateEncoderWithDifferentByteOrders() throws EncoderException {
    Encoder<DBusType, ByteBuffer> littleEndianEncoder = factory.createEncoder(Type.INT32, ByteOrder.LITTLE_ENDIAN);
    Encoder<DBusType, ByteBuffer> bigEndianEncoder = factory.createEncoder(Type.INT32, ByteOrder.BIG_ENDIAN);
  ...
  void testRegisterCustomEncoder() throws EncoderException {
    ...
    Encoder<DBusType, ByteBuffer> encoder = factory.createEncoder(Type.STRING, ByteOrder.LITTLE_ENDIAN);

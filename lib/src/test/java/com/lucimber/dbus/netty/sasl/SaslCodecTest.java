...
class SaslCodecTest {
  ...
  void testHandlerAdded() {
    ...
    verify(pipeline).addBefore(eq("SaslCodec"), eq(DBusHandlerNames.SASL_MESSAGE_DECODER), 
        any(SaslMessageDecoder.class));
    verify(pipeline).addBefore(eq("SaslCodec"), eq(DBusHandlerNames.SASL_MESSAGE_ENCODER), 
  ...
  void testHandlerRemovedWithNoSuchElementException() {
    ...
    doThrow(new NoSuchElementException("Handler not found"))
        .when(pipeline).remove(DBusHandlerNames.SASL_MESSAGE_DECODER);
  ...
  void testHandlerRemovedWithBothHandlersMissing() {
    ...
    doThrow(new NoSuchElementException("Encoder not found"))
        .when(pipeline).remove(DBusHandlerNames.SASL_MESSAGE_ENCODER);
  ...
  void testHandlerAddedWithNullContextName() {
    ...
    verify(pipeline).addBefore(isNull(), eq(DBusHandlerNames.SASL_MESSAGE_DECODER), 
        any(SaslMessageDecoder.class));
    verify(pipeline).addBefore(isNull(), eq(DBusHandlerNames.SASL_MESSAGE_ENCODER), 
        any(SaslMessageEncoder.class));
  }
}
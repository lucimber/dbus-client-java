...
class SaslAuthenticationHandlerTest {
  ...
  @Mock
  private SaslMechanism mockMechanism;
  @Mock
  private SaslMechanism mockMechanism2;
  ...
  void testSuccessfulAuthenticationFlow() throws Exception {
    when(mockMechanism.getInitialResponseAsync(any())).thenReturn(
        ImmediateEventExecutor.INSTANCE.newSucceededFuture("initialdata"));
  ...
  void testDataCommandHandling() throws Exception {
    ...
    when(mockMechanism.processChallengeAsync(any(), eq("challenge"))).thenReturn(
        ImmediateEventExecutor.INSTANCE.newSucceededFuture("response"));
  ...
  void testMechanismInitializationFailure() throws Exception {
    ...
    handler = new SaslAuthenticationHandler(List.of(mockMechanism, new AnonymousSaslMechanism()));
    channel = new EmbeddedChannel(handler);
  ...
  void testNoCompatibleMechanism() {
    ...
    // tryNextMechanism doesn't call failAuthentication when the mechanism is not in the server list.
  ...
  void testProcessChallengeFailure() throws Exception {
    when(mockMechanism.getInitialResponseAsync(any())).thenReturn(
        ImmediateEventExecutor.INSTANCE.newSucceededFuture(null));
    Future<String> failedFuture = ImmediateEventExecutor.INSTANCE.newFailedFuture(
  ...
  void testProcessChallengeReturnsNull() throws Exception {
    when(mockMechanism.getInitialResponseAsync(any())).thenReturn(
        ImmediateEventExecutor.INSTANCE.newSucceededFuture(null));
    when(mockMechanism.processChallengeAsync(any(), any())).thenReturn(
        ImmediateEventExecutor.INSTANCE.newSucceededFuture(null));

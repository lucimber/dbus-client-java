...
class SaslMessageDecoderTest {
  ...
  var input = Unpooled.copiedBuffer("DATA abc123\r\n", java.nio.charset.StandardCharsets.US_ASCII);
  ...
  var channel = new EmbeddedChannel(new SaslMessageDecoder());
  var input = Unpooled.copiedBuffer("OK 1234567890\r\n", java.nio.charset.StandardCharsets.US_ASCII);
  assertTrue(channel.writeInbound(input));
  SaslMessage msg = channel.readInbound();
  ...
  var channel = new EmbeddedChannel(new SaslMessageDecoder());
  var input = Unpooled.copiedBuffer("REJECTED EXTERNAL ANONYMOUS\r\n", java.nio.charset.StandardCharsets.US_ASCII);
  assertTrue(channel.writeInbound(input));
  SaslMessage msg = channel.readInbound();
  ...
  var channel = new EmbeddedChannel(new SaslMessageDecoder());
  var input = Unpooled.copiedBuffer("AGREE_UNIX_FD\r\n", java.nio.charset.StandardCharsets.US_ASCII);
  assertTrue(channel.writeInbound(input));
  SaslMessage msg = channel.readInbound();
  ...
  var channel = new EmbeddedChannel(new SaslMessageDecoder());
  var input = Unpooled.copiedBuffer("FOOBAR something\r\n", java.nio.charset.StandardCharsets.US_ASCII);
  ...
  var channel = new EmbeddedChannel(new SaslMessageDecoder());
  var input = Unpooled.copiedBuffer("DATA incomplete", java.nio.charset.StandardCharsets.US_ASCII);

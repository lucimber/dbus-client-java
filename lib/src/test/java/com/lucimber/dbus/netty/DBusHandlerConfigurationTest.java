...
class DBusHandlerConfigurationTest {
  ...
  @Mock
  private RealityCheckpoint mockRealityCheckpoint;
  ...
  void testInitializePipelineWithAllHandlers() {
    ...
    assertTrue(pipeline.get(DBusHandlerNames.SASL_INITIATION_HANDLER) instanceof SaslInitiationHandler);
    ...
    assertTrue(pipeline.get(DBusHandlerNames.SASL_AUTHENTICATION_HANDLER) instanceof SaslAuthenticationHandler);
    ...
    assertTrue(pipeline.get(DBusHandlerNames.OUTBOUND_MESSAGE_ENCODER) instanceof OutboundMessageEncoder);
    ...
    assertTrue(pipeline.get(DBusHandlerNames.INBOUND_MESSAGE_DECODER) instanceof InboundMessageDecoder);
    
    assertNotNull(pipeline.get(DBusHandlerNames.DBUS_MANDATORY_NAME_HANDLER));
    assertTrue(pipeline.get(DBusHandlerNames.DBUS_MANDATORY_NAME_HANDLER) instanceof DBusMandatoryNameHandler);
    
    assertNotNull(pipeline.get(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER));
    assertTrue(pipeline.get(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER) instanceof ConnectionCompletionHandler);
    
    // Reconnection manager
    assertNotNull(pipeline.get("ReconnectionHandlerManager"));
    assertTrue(pipeline.get("ReconnectionHandlerManager") instanceof ReconnectionHandlerManager);
  ...
  void testInitializePipelineWithoutAppLogicHandler() {
    ...
    DBusHandlerConfiguration.initializePipeline(
        channel.pipeline(), connectPromise, null);
  ...
  void testGetSaslHandlers() {
    ...
    Map<String, Supplier<ChannelHandler>> saslHandlers = DBusHandlerConfiguration.getSaslHandlers();
    ...
    ChannelHandler initiationHandler = saslHandlers.get(DBusHandlerNames.SASL_INITIATION_HANDLER).get();
    ...
    ChannelHandler authHandler = saslHandlers.get(DBusHandlerNames.SASL_AUTHENTICATION_HANDLER).get();
  ...
  void testHandlerOrderConsistency() {
    ...
    assertTrue(saslInitPos < byteLoggerPos, 
        "SASL handlers should come before D-Bus handlers");
  }

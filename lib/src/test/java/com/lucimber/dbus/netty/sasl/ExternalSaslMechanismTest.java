...
class ExternalSaslMechanismTest {
  ...
  void setUp() {
    ...
    when(eventLoop.newPromise()).thenAnswer(invocation -> new DefaultPromise<>(GlobalEventExecutor.INSTANCE));
  }
  ...
  void testInitWithValidAuthorizationId() throws SaslMechanismException {
    try (MockedStatic<AuthorizationIdResolver> mockedResolver = mockStatic(AuthorizationIdResolver.class)) {
  ...
  void testInitWithResolverException() {
    try (MockedStatic<AuthorizationIdResolver> mockedResolver = mockStatic(AuthorizationIdResolver.class)) {
      mockedResolver.when(AuthorizationIdResolver::resolve)
  ...
  void testGetInitialResponseAsyncWithNullAuthorizationId() throws Exception {
    ...
    try (MockedStatic<AuthorizationIdResolver> mockedResolver = mockStatic(AuthorizationIdResolver.class)) {
      ...
      java.lang.reflect.Field authIdField = ExternalSaslMechanism.class.getDeclaredField("authorizationId");
  ...
  void testProcessChallengeAsync() throws Exception {
    try (MockedStatic<AuthorizationIdResolver> mockedResolver = mockStatic(AuthorizationIdResolver.class)) {
      ...
      Future<String> responseFuture = mechanism.processChallengeAsync(context, "unexpected_challenge");
      
      Exception exception = assertThrows(Exception.class, 
          () -> responseFuture.get(5, TimeUnit.SECONDS));
      assertTrue(exception.getCause() instanceof SaslMechanismException);
      assertTrue(exception.getCause().getMessage().contains("EXTERNAL does not support challenges"));

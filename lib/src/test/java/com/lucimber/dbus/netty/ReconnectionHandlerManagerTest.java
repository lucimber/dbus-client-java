...
final class ReconnectionHandlerManagerTest {
    ...
    void testReconnectionHandlersReaddRequiredEvent() {
        ...
        ConnectionCompletionHandler completionHandler =
                spy(new ConnectionCompletionHandler(oldPromise));

        // Add the completion handler to pipeline
        channel.pipeline()
                .addAfter(
                        channel.pipeline().names().get(0),
                        DBusHandlerNames.CONNECTION_COMPLETION_HANDLER,
                        completionHandler);

        // Fire event
        channel.pipeline()
                .fireUserEventTriggered(DBusChannelEvent.RECONNECTION_HANDLERS_READD_REQUIRED);
    ...
    void testHandlerWithNullConnectPromise() {
        ...
        ReconnectionHandlerManager handlerWithNullPromise =
                new ReconnectionHandlerManager(nullPromise);
        ...
        ConnectionCompletionHandler completionHandler =
                spy(new ConnectionCompletionHandler(initialPromise));
        testChannel
                .pipeline()
                .addFirst(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER, completionHandler);
    ...
    void testEventPropagation() {
        ...
        assertDoesNotThrow(
                () -> {
                    channel.pipeline()
                            .fireUserEventTriggered(DBusChannelEvent.RECONNECTION_STARTING);
                    channel.pipeline()
                            .fireUserEventTriggered(
                                    DBusChannelEvent.RECONNECTION_HANDLERS_READD_REQUIRED);
    ...
    void testMultipleManagersInPipeline() {
        ...
        ConnectionCompletionHandler completionHandler =
                spy(new ConnectionCompletionHandler(initialPromise));
        channel.pipeline()
                .addLast(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER, completionHandler);

...
class SignalHandlingIntegrationTest extends DBusIntegrationTestBase {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(SignalHandlingIntegrationTest.class);
    ...
    void testSignalHandlerCreation() throws Exception {
        ...
        try {
            ...
            SignalCaptureHandler signalHandler = new SignalCaptureHandler(
                "NameOwnerChanged", 
                capturedSignal, 
                signalLatch
            );
            ...
            // Test creating AddMatch method call (but not sending since sendRequest is not implemented)
            OutboundMethodCall addMatch = OutboundMethodCall.Builder
                .create()
                ...
                .withBody(DBusSignature.valueOf("s"), List.of(DBusString.valueOf(
    ...
    void testMultipleSignalHandlers() throws Exception {
        ...
        try {
            ...
            MultiSignalCaptureHandler signalHandler = new MultiSignalCaptureHandler(
                signalCount, 
                signalLatch
            );
            ...
            for (String rule : matchRules) {
                OutboundMethodCall addMatch = OutboundMethodCall.Builder
                    ...
                    .withDestination(DBusString.valueOf("org.freedesktop.DBus"))
                    .withInterface(DBusString.valueOf("org.freedesktop.DBus"))
                    .withBody(DBusSignature.valueOf("s"), List.of(DBusString.valueOf(rule)))
                    .withReplyExpected(true)
                    .build();
    ...
    void testSignalFilteringSetup() throws Exception {
        ...
        try {
            ...
            FilteredSignalCaptureHandler signalHandler = new FilteredSignalCaptureHandler(
                "com.lucimber.test.FilterTest",
                capturedSignal, 
                signalLatch
            );
    ...
    private static class SignalCaptureHandler extends AbstractInboundHandler {
        ...
        public SignalCaptureHandler(String expectedMember, 
    ...
    private static class MultiSignalCaptureHandler extends AbstractInboundHandler {
        ...
        public MultiSignalCaptureHandler(AtomicReference<Integer> signalCount, 
                                       CountDownLatch latch) {
    ...
    private static class FilteredSignalCaptureHandler extends AbstractInboundHandler {
        ...
        public FilteredSignalCaptureHandler(String expectedName,
                                          AtomicReference<InboundSignal> capturedSignal, 
                                          CountDownLatch latch) {

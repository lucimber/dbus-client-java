...
class ServiceInteractionIntegrationTest extends DBusIntegrationTestBase {
    ...
    void testServiceDiscovery() throws Exception {
        ...
        try {
            ...
            boolean foundDBusService = services.stream()
    ...
    void testServiceActivation() throws Exception {
        ...
        try {
            ...
            OutboundMethodCall listActivatableNames = OutboundMethodCall.Builder
                .create()
                .withSerial(connection.getNextSerial())
                .withPath(DBusObjectPath.valueOf("/org/freedesktop/DBus"))
            ...
            CompletableFuture<InboundMessage> future = connection.sendRequest(listActivatableNames).toCompletableFuture();
            InboundMessage response = future.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
    ...
    void testDetailedIntrospection() throws Exception {
        ...
        try {
            ...
            CompletableFuture<InboundMessage> future = connection.sendRequest(introspect).toCompletableFuture();
            InboundMessage response = future.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
            ...
            assertTrue(xml.contains("org.freedesktop.DBus.Introspectable"), "Should contain Introspectable interface");
    ...
    void testServiceOwnership() throws Exception {
        ...
        try {
            ...
            CompletableFuture<InboundMessage> future = connection.sendRequest(requestName).toCompletableFuture();
            InboundMessage response = future.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
            ...
            assertTrue(resultCode >= 1 && resultCode <= 4, 
                "Should get valid RequestName result code: " + resultCode);
            
            LOGGER.info("✓ Successfully requested service name: {} (result: {})", testServiceName, resultCode);
            ...
            OutboundMethodCall nameHasOwner = OutboundMethodCall.Builder
                ...
                .withDestination(DBusString.valueOf("org.freedesktop.DBus"))
                .withInterface(DBusString.valueOf("org.freedesktop.DBus"))
                .withBody(DBusSignature.valueOf("s"), List.of(DBusString.valueOf(testServiceName)))
                .withReplyExpected(true)
                .build();

            CompletableFuture<InboundMessage> checkFuture = connection.sendRequest(nameHasOwner).toCompletableFuture();
            InboundMessage checkResponse = checkFuture.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
            ...
            com.lucimber.dbus.type.DBusBoolean hasOwner = (com.lucimber.dbus.type.DBusBoolean) checkPayload.get(0);
            ...
            CompletableFuture<InboundMessage> releaseFuture = connection.sendRequest(releaseName).toCompletableFuture();
            InboundMessage releaseResponse = releaseFuture.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
    ...
    void testConnectionUniqueName() throws Exception {
        ...
        try {
            ...
            try {
                CompletableFuture<InboundMessage> userFuture = connection.sendRequest(getConnectionUnixUser).toCompletableFuture();
                InboundMessage userResponse = userFuture.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
            ...
            for (String path : paths) {
                try {
                    ...
                    if (response instanceof InboundMethodReturn methodReturn) {
                        ...
                        LOGGER.info("✓ Successfully introspected path: {} ({} chars)", path, xml.length());
                        
                        // Basic validation
                        assertTrue(xml.contains("<node"), "Should contain node element for path: " + path);
                    }
                    
                } catch (Exception e) {
                    LOGGER.debug("Introspection failed for path {} (may be expected): {}", path, e.getMessage());
                }
            }

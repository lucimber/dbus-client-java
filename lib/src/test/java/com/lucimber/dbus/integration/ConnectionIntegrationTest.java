...
class ConnectionIntegrationTest extends DBusIntegrationTestBase {
    ...
    void testBasicConnectionLifecycle() throws Exception {
        ...
        if (isRunningInContainer()) {
            ...
            try {
                ...
                connection =
                        new NettyConnection(
                                new DomainSocketAddress("/tmp/dbus-test-socket"), config);
                ...
                LOGGER.info(
                        "✓ Successfully connected via Unix socket with EXTERNAL authentication");
        ...
        if (connection == null) {
            try {
                ...
                connection = new NettyConnection(new InetSocketAddress(host, port), config);
        ...
        if (connection == null && lastException != null) {
            throw new AssertionError(
                    "All connection attempts failed. Last error: " + lastException.getMessage(),
                    lastException);
        }
    ...
    void testDBusMethodCall() throws Exception {
        ...
        try {
            ...
            CompletableFuture<InboundMessage> responseFuture =
                    connection.sendRequest(methodCall).toCompletableFuture();
            InboundMessage response =
                    responseFuture.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS);
    ...
    void testConcurrentConnections() throws Exception {
        ...
        try {
            ...
            assertDoesNotThrow(
                    () -> allConnected.get(DEFAULT_TIMEOUT.toMillis(), TimeUnit.MILLISECONDS));

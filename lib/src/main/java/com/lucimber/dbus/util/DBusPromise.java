 ...
 * <p>
 * This class provides a fluent API for handling D-Bus responses with
...
public class DBusPromise<T> {
    ...
    public DBusPromise<T> timeout(Duration duration) {
        ...
        CompletableFuture.delayedExecutor(duration.toMillis(), TimeUnit.MILLISECONDS)
            .execute(() -> timeoutFuture.completeExceptionally(
                new DBusTimeoutException("Operation timed out after " + duration)
            ));
    ...
    public DBusPromise<List<DBusType>> mapReturn() {
        return new DBusPromise<>(stage.thenApply(value -> {
            if (!(value instanceof InboundMessage)) {
                throw new IllegalStateException("Expected InboundMessage but got " + value.getClass());
            }
            ...
            throw new IllegalStateException("Unexpected message type: " + message.getClass());
        }));
    }
            ...
            if (value instanceof List) {
                ...
                if (!list.isEmpty()) {
                    ...
                    throw new ClassCastException(
                        ...
                        " but got " + first.getClass().getSimpleName()
                    );
                }

 ...
 *
 * <p>This utility provides sophisticated frame recovery mechanisms including:
 *
 * <ul>
 *   <li>Frame corruption detection and diagnostics
 *   <li>Frame boundary synchronization after corruption
 *   <li>Partial frame recovery when possible
 *   <li>Detailed error context for debugging
 *   <li>Statistics collection for monitoring frame health
 * </ul>
 *
 * <p>The frame recovery manager is designed to be stateless and thread-safe, making it suitable for
 * use across multiple connections and decoder instances.
 */
public final class FrameRecoveryManager {
    ...
    public static FrameAnalysis analyzeFrame(ByteBuffer buffer) {
        ...
        if (buffer == null || !buffer.hasRemaining()) {
            return FrameAnalysis.createInvalid(
                    "Buffer is null or empty", CorruptionType.BUFFER_UNDERRUN);
        }
        ...
        try {
            ...
            if (buffer.remaining() < MIN_HEADER_LENGTH) {
                return FrameAnalysis.createInvalid(
                        String.format(
                                "Buffer too small for header: %d bytes (need %d)",
            ...
            if (endianFlag == LITTLE_ENDIAN_FLAG) {
                ...
                return FrameAnalysis.createInvalid(
                        String.format(
                                "Invalid endianness flag: 0x%02x (expected 'l' or 'B')",
                                endianFlag),
            ...
            if (protocolVersion != 1) {
                return FrameAnalysis.createInvalid(
                        String.format(
                                "Unsupported protocol version: %d (expected 1)", protocolVersion),
            ...
            if (bodyLength < 0 || bodyLength > MAX_MESSAGE_SIZE) {
                return FrameAnalysis.createInvalid(
                        String.format(
                                "Invalid body length: %d (max %d)", bodyLength, MAX_MESSAGE_SIZE),
            ...
            if (headerFieldLength < 0 || headerFieldLength > MAX_MESSAGE_SIZE) {
                return FrameAnalysis.createInvalid(
                        String.format(
                                "Invalid header field length: %d (max %d)",
                                headerFieldLength, MAX_MESSAGE_SIZE),
                        CorruptionType.INVALID_LENGTH);
            }
            ...
            if (totalFrameSize > MAX_MESSAGE_SIZE) {
                return FrameAnalysis.createInvalid(
                        String.format(
                                ...
                                totalFrameSize, MAX_MESSAGE_SIZE),
            ...
            if (buffer.remaining() < totalFrameSize) {
                return FrameAnalysis.createPartial(
                        String.format(
                                "Incomplete frame: have %d bytes, need %d",
                                buffer.remaining(), totalFrameSize),
                        totalFrameSize - buffer.remaining());
            }

            // Frame appears valid
            return FrameAnalysis.createValid(
                    totalFrameSize, byteOrder, messageType, bodyLength, headerFieldLength);
    ...
    public static FrameDiagnostic createDiagnostic(ByteBuffer buffer, int maxBytes) {
        if (buffer == null) {
            return new FrameDiagnostic(
                    "Buffer is null",
                    "",
                    FrameAnalysis.createInvalid("Null buffer", CorruptionType.BUFFER_UNDERRUN));
        }
    ...
    public static class FrameAnalysis {
        ...
        public static FrameAnalysis createValid(
                int frameSize,
                ByteOrder byteOrder,
                int messageType,
                int bodyLength,
        ...
        public static FrameAnalysis createInvalid(
                String errorMessage, CorruptionType corruptionType) {
            return new FrameAnalysis(
                    false, false, errorMessage, corruptionType, -1, 0, null, -1, -1, -1);
        }
        ...
        public static FrameAnalysis createPartial(String errorMessage, int missingBytes) {
            return new FrameAnalysis(
                    ...
                    true,
                    errorMessage,
                    CorruptionType.INCOMPLETE_FRAME,
                    -1,
                    missingBytes,
                    null,
                    -1,
    ...
    public static class FrameStatistics {
        ...
        public FrameStatistics(
                long totalFrames, long corruptedFrames, long recoveredFrames, long skippedFrames) {

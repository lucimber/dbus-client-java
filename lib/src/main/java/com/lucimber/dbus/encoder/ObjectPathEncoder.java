...
public final class ObjectPathEncoder implements Encoder<DBusObjectPath, ByteBuffer> {
  ...
  public EncoderResult<ByteBuffer> encode(DBusObjectPath value, int offset) throws EncoderException {
    ...
    try {
      // Calculate padding
      int padding = EncoderUtils.calculateAlignmentPadding(Type.OBJECT_PATH.getAlignment(), offset);
      ...
      int totalSize = padding + lengthResult.getProducedBytes() + bytes.length + NUL_TERMINATOR_LENGTH;

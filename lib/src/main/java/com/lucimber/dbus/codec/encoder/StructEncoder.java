...
public final class StructEncoder implements Encoder<DBusStruct, ByteBuffer> {
    ...
    public EncoderResult<ByteBuffer> encode(DBusStruct struct, int offset) throws EncoderException {
        ...
        try {
            int producedBytes = 0;
            int padding =
                    EncoderUtils.calculateAlignmentPadding(Type.STRUCT.getAlignment(), offset);
            ...
            for (int i = 0; i < values.size(); i++) {
                ...
                EncoderResult<ByteBuffer> partResult =
                        EncoderUtils.encode(value, interimOffset, order);

 ...
 * Both UID (Unix) and SID (Windows) are supported by this resolver.
 */
  ...
  private static String getWindowsSid() throws SaslMechanismException {
    try {
      ...
      try {
        ...
        throw new SaslMechanismException("Failed to retrieve Windows SID from either domain or local user.", e2);
      }
    }
  }

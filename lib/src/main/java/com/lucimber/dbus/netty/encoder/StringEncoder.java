...
/**
 * An encoder which encodes a string to the D-Bus marshalling format.
 *
 * @see Encoder
 * @see DBusString
 */
public final class StringEncoder implements Encoder<DBusString, ByteBuf> {
   ...
   */
  public StringEncoder(final ByteBufAllocator allocator, final ByteOrder order) {
  ...
  private static void logResult(final DBusString value, final int offset, final int padding,
    ...
    LoggerUtils.debug(LOGGER, MARKER, () -> {
      final String s = "STRING: %s; Offset: %d; Padding: %d; Produced bytes: %d;";
      return String.format(s, value, offset, padding, producedBytes);
    });
  }

  @Override
  public EncoderResult<ByteBuf> encode(final DBusString value, final int offset) throws EncoderException {
    Objects.requireNonNull(value, "value must not be null");
    final ByteBuf buffer = allocator.buffer();
    try {
      int producedBytes = 0;
      final int padding = EncoderUtils.applyPadding(buffer, offset, Type.STRING);

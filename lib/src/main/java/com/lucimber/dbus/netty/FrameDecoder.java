...
final class FrameDecoder extends ByteToMessageDecoder {
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        LOGGER.error(
                LoggerUtils.HANDLER_LIFECYCLE, "Encountered an exception while decoding.", cause);
    ...
    private boolean canDecodeHeaderFields(ByteBuf in) {
        ...
        if (fieldsLen > Integer.MAX_VALUE - padding) {
            throw new TooLongFrameException(
                    "Header fields length with padding too large: " + fieldsLen + " + " + padding);
        }
        ...
        if (fieldsLen > Integer.MAX_VALUE - 4) {
            throw new TooLongFrameException(
                    "Header fields length too large for buffer check: " + fieldsLen);
        }

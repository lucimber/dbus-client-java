 ...
 * <p>
 * This strategy handles connections over Unix domain sockets using native
 * transports (Epoll on Linux, KQueue on macOS) for optimal performance.
 */
      ...
      if (future.isSuccess()) {
        ...
        nettyConnectPromise.addListener(connectResult -> {
          if (connectResult.isSuccess()) {
            LOGGER.debug("D-Bus handshake completed for Unix domain socket");
            // Notify context that connection is fully established
            context.onConnectionEstablished();
            handleFuture.complete(handle);
          } else {
            LOGGER.error("D-Bus handshake failed for Unix domain socket", connectResult.cause());
            context.onError(connectResult.cause());
            handleFuture.completeExceptionally(connectResult.cause());
          }
        });
      } else {
        LOGGER.error("Failed to establish Unix domain socket connection to {}", address, future.cause());

 ...
 * This class maintains the definitive order and creation logic for all handlers,
 * ensuring perfect synchronization between initial setup and reconnection scenarios.
 */
public final class DBusHandlerConfiguration {
  ...
  /**
   * Defines the complete handler pipeline order and creation logic.
  ...
  private static Map<String, Supplier<ChannelHandler>> createHandlerMap(
    ...
    handlers.put(DBusHandlerNames.NETTY_BYTE_LOGGER, 
    ...
    handlers.put(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER, 
            () -> new ConnectionCompletionHandler(connectPromise));
    
    // Reconnection management handler
    handlers.put("ReconnectionHandlerManager", () -> new ReconnectionHandlerManager(connectPromise));
  ...
  public static Map<String, Supplier<ChannelHandler>> getSaslHandlers() {
    ...
    saslHandlers.put(DBusHandlerNames.SASL_AUTHENTICATION_HANDLER, SaslAuthenticationHandler::new);
    return saslHandlers;
  }

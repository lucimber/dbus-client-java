...
/**
 * Netty-specific implementation of ConnectionContext that bridges
 * the strategy pattern with the existing NettyConnection infrastructure.
 */
final class NettyConnectionContext implements ConnectionContext {
  ...
  NettyConnectionContext(Pipeline pipeline,
  ...
  /**
   * Gets the underlying NettyConnection instance.
   * This method is used by strategy implementations to access the connection.
   *
  ...
  public void onMessageReceived(InboundMessage message) {
    LOGGER.debug("Message received via strategy pattern: {}", message.getClass().getSimpleName());

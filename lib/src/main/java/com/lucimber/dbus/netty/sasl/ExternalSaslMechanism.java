...
public final class ExternalSaslMechanism implements SaslMechanism {
  ...
  public Future<String> getInitialResponseAsync(ChannelHandlerContext ctx) {
    ...
    findWorkerExecutor(ctx).execute(() -> {
      if (authorizationId == null) {
        promise.setFailure(new SaslMechanismException("Authorization ID not initialized."));
        return;
      }
      try {
        String hexResponse = SaslUtil.hexEncode(authorizationId.getBytes(StandardCharsets.US_ASCII));
        promise.setSuccess(hexResponse);
        this.complete = true;
      } catch (Exception e) {
        promise.setFailure(new SaslMechanismException("Failed to hex-encode EXTERNAL ID", e));
      }
    });
    return promise;
  }
  ...
  public Future<String> processChallengeAsync(ChannelHandlerContext ctx, String challenge) {
    Promise<String> promise = ctx.executor().newPromise();
    promise.setFailure(new SaslMechanismException("EXTERNAL does not support challenges. Got: " + challenge));
    return promise;
  }

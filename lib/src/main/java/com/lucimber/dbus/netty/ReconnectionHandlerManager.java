 ...
 * This handler listens for reconnection events and re-adds handlers that were
 * removed during the connection process, such as SASL handlers.
 */
public final class ReconnectionHandlerManager extends ChannelInboundHandlerAdapter {
  ...
  private void handleReconnectionHandlersReaddRequired(ChannelHandlerContext ctx) {
    LOGGER.debug(LoggerUtils.CONNECTION, "Re-adding handlers that were removed during connection");
  ...
  private void updateConnectionCompletionHandler(ChannelHandlerContext ctx) {
    ConnectionCompletionHandler completionHandler = 
        (ConnectionCompletionHandler) ctx.pipeline().get(DBusHandlerNames.CONNECTION_COMPLETION_HANDLER);
    if (completionHandler != null) {
      completionHandler.reset(connectPromise);
      LOGGER.debug(LoggerUtils.HANDLER_LIFECYCLE, "Updated ConnectionCompletionHandler with new promise");
    }
  }
}
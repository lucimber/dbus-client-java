 ...
 * This class encapsulates connection state management and ensures thread-safe state transitions.
 */
public class ConnectionLifecycleManager {
  ...
  public ConnectionLifecycleManager(ConnectionConfig config, String connectionId) {
    ...
    this.connectionCircuitBreaker = errorRecoveryManager.createCircuitBreaker(
        "connection-" + connectionId, cbConfig);
  }
    ...
    return connectionCircuitBreaker.execute(() -> 
        connectionSupplier.connect()
            ...
            .whenComplete((handle, error) -> {
              connecting.set(false);
              if (error != null) {
                LOGGER.error("Connection failed for {}", connectionId, error);
                updateState(ConnectionState.FAILED);
              } else {
                LOGGER.info("Connection established for {}", connectionId);
  ...
  public CompletionStage<Void> disconnect(DisconnectionSupplier disconnectionSupplier) {
    ...
    return disconnectionSupplier.disconnect()
        .handle((result, error) -> {
          if (error != null) {
            LOGGER.warn("Error during disconnection for {}", connectionId, error);
          }
  ...
  private void updateState(ConnectionState newState) {
    ...
    if (oldState != newState) {
      LOGGER.debug("Connection {} state changed from {} to {}", 
          connectionId, oldState, newState);
    }
  }

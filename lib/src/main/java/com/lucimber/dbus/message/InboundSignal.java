...
public final class InboundSignal extends AbstractSignal implements InboundMessage {
    ...
    public String toString() {
        var s =
                "InboundSignal{sender='%s', serial='%s', path='%s', iface='%s', member='%s', sig='%s'}";
        var sig = getSignature().map(DBusSignature::toString).orElse("");
        return String.format(
                s, sender, getSerial(), getObjectPath(), getInterfaceName(), getMember(), sig);
    }
    ...
    public static class Builder {
        ...
        public InboundSignal build() {
            validate();
            return new InboundSignal(serial, sender, path, iface, member, signature, payload);
        }

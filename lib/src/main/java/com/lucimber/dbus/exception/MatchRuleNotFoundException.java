package com.lucimber.dbus.exception;

import com.lucimber.dbus.type.DBusString;

/**
 * The specified match rule does not exist.
 */
public final class MatchRuleNotFoundException extends AbstractException {

  private static final DBusString ERROR_NAME = DBusString.valueOf("org.freedesktop.DBus.Error.MatchRuleNotFound");

  /**
   * Creates a new instance.
   */
  public MatchRuleNotFoundException() {
    super(ERROR_NAME);
  }

  /**
   * Creates a new instance with a message.
   *
   * @param message the message
   */
  public MatchRuleNotFoundException(final DBusString message) {
    super(ERROR_NAME, message);
  }

  /**
   * Creates a new instance with a cause.
   *
   * @param cause the cause
   */
  public MatchRuleNotFoundException(final Throwable cause) {
    super(ERROR_NAME, cause);
  }

  /**
   * Creates a new instance with a message and cause.
   *
   * @param message the message
   * @param cause   the cause
   */
  public MatchRuleNotFoundException(final DBusString message, final Throwable cause) {
    super(ERROR_NAME, message, cause);
  }
}
